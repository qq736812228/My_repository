...
public class AdminMallOrderController {
    private final MallOrderRepository repository;

    public AdminMallOrderController(MallOrderRepository repository) {
        this.repository = repository;
    }

    @GetMapping
    public ApiResponse<List<MallOrder>> list() {
        return ApiResponse.ok(repository.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<MallOrder> detail(@PathVariable Long id) {
        return ApiResponse.ok(repository.findById(id).orElseThrow());
    }

    @PostMapping
    @OperationAudit(module = "订单管理", action = "新增")
    public ApiResponse<MallOrder> create(@RequestBody MallOrder body) {
        return ApiResponse.ok(repository.save(body));
    }

    @PutMapping("/{id}")
    @OperationAudit(module = "订单管理", action = "修改")
    public ApiResponse<MallOrder> update(@PathVariable Long id, @RequestBody MallOrder body) {
    ...
    @DeleteMapping("/{id}")
    @OperationAudit(module = "订单管理", action = "删除")
