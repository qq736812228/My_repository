...
@RestController
@RequestMapping("/api/admin/point-accounts")
public class AdminPointAccountController {
    private final PointAccountRepository repository;

    public AdminPointAccountController(PointAccountRepository repository) {
        this.repository = repository;
    }

    @GetMapping
    public ApiResponse<List<PointAccount>> list() {
        return ApiResponse.ok(repository.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<PointAccount> detail(@PathVariable Long id) {
        return ApiResponse.ok(repository.findById(id).orElseThrow());
    }

    @PostMapping
    @OperationAudit(module = "积分账户", action = "新增")
    public ApiResponse<PointAccount> create(@RequestBody PointAccount body) {
        return ApiResponse.ok(repository.save(body));
    }

    @PutMapping("/{id}")
    @OperationAudit(module = "积分账户", action = "修改")
    public ApiResponse<PointAccount> update(@PathVariable Long id, @RequestBody PointAccount body) {
    ...
    @DeleteMapping("/{id}")
    @OperationAudit(module = "积分账户", action = "删除")
