...
import com.obai.platform.repository.NotificationMessageRepository;
...
public class AdminNotificationMessageController {
    ...
    public AdminNotificationMessageController(NotificationMessageRepository repository) {
    ...
    @GetMapping("/{id}")
    public ApiResponse<NotificationMessage> detail(@PathVariable Long id) {
        return ApiResponse.ok(repository.findById(id).orElseThrow());
    }

    @PostMapping
    @OperationAudit(module = "消息管理", action = "新增")
    public ApiResponse<NotificationMessage> create(@RequestBody NotificationMessage body) {
        return ApiResponse.ok(repository.save(body));
    }

    @PutMapping("/{id}")
    @OperationAudit(module = "消息管理", action = "修改")
    public ApiResponse<NotificationMessage> update(@PathVariable Long id, @RequestBody NotificationMessage body) {
    ...
    @DeleteMapping("/{id}")
    @OperationAudit(module = "消息管理", action = "删除")
