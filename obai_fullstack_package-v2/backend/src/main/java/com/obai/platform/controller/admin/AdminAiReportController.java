...
import com.obai.platform.repository.AiReportRepository;
...
public class AdminAiReportController {
    ...
    public AdminAiReportController(AiReportRepository repository) {
    ...
    @GetMapping("/{id}")
    public ApiResponse<AiReport> detail(@PathVariable Long id) {
        return ApiResponse.ok(repository.findById(id).orElseThrow());
    }

    @PostMapping
    @OperationAudit(module = "AI报告", action = "新增")
    public ApiResponse<AiReport> create(@RequestBody AiReport body) {
        return ApiResponse.ok(repository.save(body));
    }

    @PutMapping("/{id}")
    @OperationAudit(module = "AI报告", action = "修改")
    public ApiResponse<AiReport> update(@PathVariable Long id, @RequestBody AiReport body) {
    ...
    @DeleteMapping("/{id}")
    @OperationAudit(module = "AI报告", action = "删除")
