...
@RestController
@RequestMapping("/api/admin/detection-reports")
public class AdminDetectionReportController {
    private final DetectionReportRepository repository;

    public AdminDetectionReportController(DetectionReportRepository repository) {
        this.repository = repository;
    }

    @GetMapping
    public ApiResponse<List<DetectionReport>> list() {
        return ApiResponse.ok(repository.findAll());
    }

    @GetMapping("/{id}")
    public ApiResponse<DetectionReport> detail(@PathVariable Long id) {
        return ApiResponse.ok(repository.findById(id).orElseThrow());
    }

    @PostMapping
    @OperationAudit(module = "检测报告", action = "新增")
    public ApiResponse<DetectionReport> create(@RequestBody DetectionReport body) {
        return ApiResponse.ok(repository.save(body));
    }

    @PutMapping("/{id}")
    @OperationAudit(module = "检测报告", action = "修改")
    public ApiResponse<DetectionReport> update(@PathVariable Long id, @RequestBody DetectionReport body) {
    ...
    @DeleteMapping("/{id}")
    @OperationAudit(module = "检测报告", action = "删除")
